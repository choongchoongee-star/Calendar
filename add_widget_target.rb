...
project.objects.select { |obj| obj.isa == 'PBXFileReference' && obj.path && (obj.path.include?('CalendarWidget') || obj.path.include?('entitlements')) }.each(&:remove_from_project)

swift_ref = widget_group.new_file('CalendarWidget.swift')
widget_ent_ref = widget_group.new_file('CalendarWidget.entitlements')
app_ent_ref = app_group.new_file('App.entitlements')
...
project.save
puts "Successfully applied entitlements and refined widget bundling."
